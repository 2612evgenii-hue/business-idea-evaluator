...
echo "== 2. Python syntax (validate_input.py) =="
python3 -m py_compile scripts/validate_input.py && echo "PASS"

echo "== 3. Bash syntax (install-agents.sh) =="
bash -n scripts/install-agents.sh && echo "PASS"

echo "== 4. JSON validation (sample) =="
python3 scripts/validate_input.py examples/sample-input.json

echo "== 5. BRS computation (sample) =="
...
echo "== 8. Risk-direction regression =="
python3 - <<'PY'
import json, subprocess, tempfile, os
base = json.load(open("examples/sample-input.json"))
...
echo "== 9. Blocking cap (legal_safety=2 -> cap 35) =="
python3 - <<'PY'
import json, subprocess, tempfile, os
d = json.load(open("examples/sample-input.json"))
